...
from modules.rewards_3 import get_self_critical_reward, init_scorer, sentence_score, corpus_score
...
def generate_3d_array(step=0.1, target_sum=1.0):
    array = []
    for i in np.arange(0, target_sum + step, step):
        for j in np.arange(0, target_sum - i + step, step):
            k = target_sum - i - j
            if k >= 0:
...
class BaseTrainer(object):
    def __init__(self, model, criterion, metric_ftns, ve_optimizer, ed_optimizer, args):
        ...
        self.preference_vectors_three_dimensions = generate_3d_array()
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        hyp = self.preference_vectors_three_dimensions[self.index]
        print('b4:', hyp[0], 'rg:', hyp[1], 'mtr:', hyp[2])
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.train_dataloader):
            ...
            gts = reports_ids[:, 1:]
            reward1, reward2, reward3 = get_self_critical_reward(greedy_res, gts, gen_result, hyp, self.model.tokenizer, images_id)



            reward1, reward2, reward3 = torch.from_numpy(reward1).to(sample_logprobs), torch.from_numpy(reward2).to(sample_logprobs), torch.from_numpy(reward3).to(sample_logprobs)
            ...
            loss_rl_3 = self.criterion(sample_logprobs, gen_result.data, reward3)
            ...
            loss = 0.01 * loss_nll + 0.99 * (hyp[0] * loss_rl_1 + hyp[1] * loss_rl_2 + hyp[2] * loss_rl_3)

            train_loss += loss.item()
            # if epoch > 40:
            train_loss_rl += 0.99 * (hyp[0] * loss_rl_1 + hyp[1] * loss_rl_2 + hyp[2] * loss_rl_3).item()

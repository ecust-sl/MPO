...
class Transformer(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        ...
        # mask = (tgt.data > 0)
        # mask[:, 0] += True
        # embeddings = pack_wrapper(self.norm, embeddings, mask)
...
class MultiThreadMemory(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, topk=32):
        ...
        self.d_k = d_model // h
        self.h = h
        # self.svd_linear = lora.SVDLinear(d_model, d_model, r=16)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, r = 16):
        ...
        self.d_k = d_model // h
        self.h = h
        self.svd_linear = lora.SVDLinear(d_model, d_model, r=r)
        self.linears = clones(self.svd_linear, 4)
        # self.linears = clones(nn.Linear(d_model, d_model), 4)
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)
...
class PositionwiseFeedForward(nn.Module):
    def __init__(self, d_model, d_ff, dropout=0.1, r = 16):
        super(PositionwiseFeedForward, self).__init__()
        self.w_1 = nn.Linear(d_model, d_ff)
        self.w_2 = nn.Linear(d_ff, d_model)
        # self.w_1 = lora.SVDLinear(d_model, d_ff, r = r)
        # self.w_2 = lora.SVDLinear(d_ff, d_model, r = r)
        self.dropout = nn.Dropout(dropout)
...
class Embeddings(nn.Module):
    def __init__(self, d_model, vocab, r = 16):
        super(Embeddings, self).__init__()
        # self.lut = lora.SVDEmbedding(vocab, d_model, r = r)
...
class BaseCMN(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn):
        c = copy.deepcopy
        attn = MultiHeadedAttention(self.num_heads, self.d_model, r = self.r)
        ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout, r = self.r)
        ...
        model = Transformer(
            ...
            nn.Sequential(Embeddings(self.d_model, tgt_vocab, r = self.r), c(position)), cmn)
    ...
    def __init__(self, args, tokenizer):
        ...
        # self.norm_vis = nn.BatchNorm1d(args.d_model)
        # self.norm_txt = nn.BatchNorm1d(args.d_model)

        self.model = self.make_model(tgt_vocab, self.cmn)
        self.logit = lora.SVDLinear(args.d_model, tgt_vocab, self.r)
